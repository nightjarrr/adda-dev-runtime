...
**Verification scope is limited.** Inside the container, verification is restricted to:
- File and path checks in `/workspace`
- Running `quality-gates` against the repo source

Live image testing — launching a container from the built image, exercising entrypoint behavior, or testing host-side launcher flows — requires the host and is a PO operation.
