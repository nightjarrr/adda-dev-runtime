...
export async function executeBranchEnsure(deps: ShellDep, store: IssueStateStore, output: ScriptOutput): Promise<void> {
    ...
    const currentBranchResult = await deps.shell.run(["git", "branch", "--show-current"], { strict: false });
    ...
    const developResult = await deps.shell.run(["gh", "issue", "develop", state.id, "-n", branchName], { strict: false });
