...
import { makeEnvelopeSchema, parseJson, ScriptStructuredError, ScriptZodValidationError } from "@adda/lib";
import { z } from "zod";
...
export async function resolveIssueBranch(deps: ShellDep, issueId: string): Promise<ResolveIssueBranchData> {
    ...
    if (resolveResult.exitCode !== 0) {
        throw new ScriptStructuredError("resolve_failed", `resolve-issue-branch failed for issue #${issueId}`, {
    ...
    try {
        ...
        throw new ScriptStructuredError("api_error", `invalid JSON from resolve-issue-branch for issue #${issueId}`);
    }
    ...
    if (!resolveParsed.success) {
        ...
        throw new ScriptStructuredError("validation_error", err.message, { verboseStderr: err.verboseStderr });
    }

    const data = resolveParsed.data;

    if (data.status === "fail") {
        throw new ScriptStructuredError(data.error.reason, data.error.message, {
