...
export async function runPr(deps: PrDeps, args: Extract<PrReviewThreadsArgs, { mode: "pr" }>): Promise<PrResult> {
    ...
    if (firstParsed.data.data.repository.pullRequest === null) {
        throw new ScriptStructuredError("pr_not_found", `PR #${args.prNumber} not found in ${owner}/${repo}`);
    }
    ...
    if (total > ceiling) {
        throw new ScriptStructuredError("scan_limit_exceeded", `scan limit exceeded: ${total} threads > ceiling ${ceiling}`, {
