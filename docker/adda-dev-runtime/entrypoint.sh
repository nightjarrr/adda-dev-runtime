...
# ======================================================================
section "Welcome to ephemeral adda dev environment"
...
bootstrap_failure() {
    printf '\n\033[1;31m==> %s\033[0m\n' "Entrypoint bootstrap failed, container cannot proceed."
...
if [[ -d /usr/local/libexec/adda-dev-runtime/entrypoint.d ]]; then
    shopt -s nullglob
    hooks=(/usr/local/libexec/adda-dev-runtime/entrypoint.d/*.sh)
    shopt -u nullglob
    if (( ${#hooks[@]} > 0 )); then
